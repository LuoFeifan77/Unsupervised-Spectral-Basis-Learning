...
class Attention(nn.Module):  # 3的部分的权重矩阵
    '''
    ...
    '''
    def __init__(self, alpha:float = 1.0, orders: int = 200 , learn_attention: bool=True):
        super().__init__()
        self.learn_attention = learn_attention
        self.orders = orders
        # self.comb_weight = nn.Parameter(torch.ones((1, depth, channels)))
        if self.learn_attention:
            self.alphas = nn.Parameter(torch.ones((orders)))
    ...
    def forward(self):
        ...
        '''
        # self.alphas = torch.stack(self.alphas)
        # att = torch.softmax(self.alphas*x)

        att = torch.softmax(self.alphas, dim=0)
...
#         self.mlp1 = nn.Sequential(
#             PointConvBlock(fdim, nfeatures, use_norm=True, use_act=True),  # 点云卷积
