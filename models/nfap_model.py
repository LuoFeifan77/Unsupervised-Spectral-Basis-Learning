...
class NFAPModel(BaseModel):  # Base model
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(NFAPModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        gs_x, gs_y = self.networks['comb'](basis_x, basis_y)  # [1, 16, 200]
        ...
        # 设置 frequency awareness couple loss
        self.loss_metrics['l_align_fre'] = 0.0

        #8.1 frequency awareness couple loss
        self.loss_metrics['l_align_fre'] = self.losses['align_loss'](Cxy, Cxy_filtering)  # key ideas

        #8.2 calculate loss
        if not self.partial:
            if Cyx != None:
                self.loss_metrics['l_align_fre'] += self.losses['align_loss'](Cyx, Cyx_filtering)  #添加双向
    ...
    def validate_single(self, data, timer):
        ...
        if self.non_isometric:
            ...
            # p2p = fmap2pointmap(Cxy_filtering, evecs_x, evecs_y)  # NNsearch
            ...
            ### using nnsearch to obtain point map
            # feat_x = F.normalize(feat_x, dim=-1, p=2)  # 仅仅对近似等距使用
            # feat_y = F.normalize(feat_y, dim=-1, p=2)

            # p2p = nn_query(feat_x, feat_y).squeeze() # nearest neighbour query
            
            # using learned filter functions to refine
            for iter in range(5):  # 迭代几次
            ...
            Pyx = evecs_y @ Cxy_filtering @ evecs_trans_x  #置换矩阵
    ...
    def validation(self, dataloader, tb_logger, update=True): # 进行验证
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(NFAPModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
