...
class BaseModel:
    ...
    def optimize_parameters(self):
        ...
        # zero grad
        for name in self.optimizers:  # 需要看一下
        ...
        # update weight
        for name in self.optimizers:   # 设计独立的学习率！ 这个学习率都是一样的
    ...
    def get_current_learning_rate(self):  # 给不同参数进行学习率的更新
        """
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        for index, data in enumerate(pbar):
            ...
            # p2p, Pyx, Cxy = self.validate_single(data, timer)  # 保存滤波器看下
            # p2p, Pyx, Cxy = to_numpy(p2p), to_numpy(Pyx), to_numpy(Cxy)

            # filter_path = os.path.join(self.opt['path']['experiments_root'])

            # filter_name_x = 'filter_x' +  str(self.curr_iter) + '.mat'
            # filter_name_y = 'filter_y' +  str(self.curr_iter) + '.mat'

            # save_filter_x = {'gs_x': gs_x}
            # sio.savemat(os.path.join(filter_path, filter_name_x), save_filter_x) # save filter
            # save_filter_y = {'gs_y': gs_y}
            # sio.savemat(os.path.join(filter_path, filter_name_y), save_filter_y) # save filter
        ...
        if self.opt.get('save_filter', False):
            
            filter_path = os.path.join(self.opt['path']['experiments_root'], 'filters')
        ...
        if len(geo_errors) != 0:
            ...
            auc, fig, pcks = self.metrics['plot_pck'](geo_errors)  # 绘制的是平局
            ...
            # display results
            logger = get_root_logger()
            logger.info(f'Val auc: {auc:.4f}')  # 记录信息
    ...
    def _setup_optimizers(self):
        def get_optimizer():
            if optim_type == 'Adam':
                return optim.Adam(optim_params, **train_opt['optims'][name])   # 给不同的网络模块赋予不同的学习率
    ...
    def _networks_to_device(self):
        ...
        for name, network in self.networks.items():
            ...
            # 不使用数据并行
            # network = DataParallel(network, device_ids=[self.opt['gpu_idx']])  #指定gpu_idx
