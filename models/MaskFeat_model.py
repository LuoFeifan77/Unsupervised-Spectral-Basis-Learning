...
class MaskFeatModel(BaseModel):  # Base model
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(MaskFeatModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        # select feature 
        feat_x, feat_y = self.networks['featconv'](feat_x, feat_y)


        # 这个地方也要调整
        Cxy, Cyx = self.networks['fmap_net'](feat_x, feat_y, evals_x, evals_y, evecs_trans_x, evecs_trans_y)  # 学习的
    ...
    def validate_single(self, data, timer):
        ...
        evecs_x_gs = evecs_x.squeeze()
        evecs_y_gs = evecs_y.squeeze()
        evecs_trans_x_gs = evecs_trans_x.squeeze()
        evecs_trans_y_gs = evecs_trans_y.squeeze()
    ...
    def validation(self, dataloader, tb_logger, update=True): # 进行验证
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(MaskFeatModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
