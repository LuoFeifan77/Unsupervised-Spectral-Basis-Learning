...
# from diff_zo.diffZO import KernelZoomOut   # 将这个放进来


@MODEL_REGISTRY.register()
class LearningBasisNNModel(BaseModel):  # Base model
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(LearningBasisNNModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        # NNsearch p2p --> functional maps C
        p2pyx = nn_query(feat_x, feat_y).squeeze()  # 这个比我的对比学习方式特别有用
        p2pxy = nn_query(feat_y, feat_x).squeeze()

        # 这里不用学习的basis
        Cxy_nn = torch.bmm(evecs_trans_y, evecs_x[:, p2pyx, :])
        Cyx_nn = torch.bmm(evecs_trans_x, evecs_y[:, p2pxy, :])
        ...
        # loss, 这个loss可以尝试去修改，特别是对后面的正则项。 
        self.loss_metrics = self.losses['surfmnet_loss'](Cxy_est, Cyx_est, evals_x, evals_y)  # for estimation, bij+orth

        # 直接对齐
        self.loss_metrics['l_align'] = self.losses['align_loss'](Cxy_nn, Cxy_est)  # 和它对应分辨率的loss 进行对齐

        #8.2 calculate loss
        if not self.partial:
            if Cyx_nn != None:
                self.loss_metrics['l_align'] += self.losses['align_loss'](Cyx_nn, Cyx_est)  #添加双向

        # self.loss_metrics = self.losses['surfmnet_loss'](Cxy_est, Cyx_est, evals_x, evals_y)  # 只能加在spectral branch
    ...
    def validation(self, dataloader, tb_logger, update=True): # 进行验证
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(LearningBasisNNModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
