...
class MultiFAPModel(BaseModel):  # Base model
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(MultiFAPModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        # C12.shape[1], C12.shape[2], device=C12.device
        ...
        # 需要添加滤波器的惩罚项
        # gs_x2 = torch.sum(gs_x**2, dim=1)
        # gs_y2 = torch.sum(gs_y**2, dim=1)

        # self.loss_metrics['l_align_filters'] = 10 * self.losses['align_loss'](gs_x2, gs_y2)  # 滤波器对齐
    ...
    def validate_single(self, data, timer):
        ...
        for _ in range(iter_num):  # 迭代几次
            ...
            Cxy_est = self.MCFP(gs_y, gs_x, Cxy_est.unsqueeze(0))  # [1, K ,K], using learned filters
            # convert functional map to point-to-point map
            p2p = fmap2pointmap(Cxy_est.squeeze() , evecs_x, evecs_y)  #[K, K], computing p2p maps
    ...
    def validation(self, dataloader, tb_logger, update=True): # 进行验证
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(MultiFAPModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
