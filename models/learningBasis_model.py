...
class LearningBasisModel(BaseModel):  # Base model
    def __init__(self, opt):
        ...
        if self.with_refine > 0:
            opt['is_train'] = True
        super(LearningBasisModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        #3 learning basis       
        # gs_x, gs_y, evecs_x_gs, evecs_y_gs, evecs_trans_x_gs, evecs_trans_y_gs, 
        ...
        # spectral branch & spatial branch loss
    ...
    def validation(self, dataloader, tb_logger, update=True): # 进行验证
        ...
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = False
        super(LearningBasisModel, self).validation(dataloader, tb_logger, update)
        if 'permutation' in self.networks:
            self.networks['permutation'].hard = False
        if 'fmap_net' in self.networks:
            self.networks['fmap_net'].bidirectional = True
